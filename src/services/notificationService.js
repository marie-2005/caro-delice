...
/**
 * Envoyer une notification email au client lorsque sa commande est acceptée/validée
 * @param {Object} orderData - Données de la commande
 * @param {string} orderData.orderId - ID de la commande
 * @param {string} orderData.customerName - Nom du client
 ...
 * @param {string} orderData.status - Statut de la commande
 */
export const sendOrderAcceptedNotification = async (orderData) => {
  try {
    // Vérifier si EmailJS est configuré
    const emailJsPublicKey = import.meta.env.VITE_EMAILJS_PUBLIC_KEY
    const emailJsServiceId = import.meta.env.VITE_EMAILJS_SERVICE_ID
    ...
    if (!emailJsPublicKey || !emailJsServiceId || !emailJsTemplateId) {
      console.warn('⚠️ EmailJS non configuré - Notification de validation non envoyée')
      return { success: false, reason: 'EmailJS non configuré' }
    }

    // Vérifier que le client a un email
    if (!orderData.customerEmail) {
      console.warn('⚠️ Pas d\'email client - Notification de validation non envoyée')
    ...
    const templateParams = {
      ...
      total: orderData.total.toLocaleString(),
      items: itemsList,
      status: 'acceptée - en préparation',
      delivery_type: orderData.deliveryType === 'livraison' 
        ? `Livraison: Chambre ${orderData.roomNumber || 'Non renseigné'}`
        : 'Retrait: Chambre C-75',
      payment_method: orderData.paymentMethod || 'Non renseigné',
      notes: orderData.notes || 'Aucune note',
      date: new Date(orderData.createdAt || new Date()).toLocaleString('fr-FR'),
      message: `Votre commande #${orderData.orderId.slice(-6)} a été acceptée et est en cours de préparation !`
    }

    // Log des paramètres avant envoi
    console.log('📧 Envoi de notification de validation au client...', {
      serviceId: emailJsServiceId,
      templateId: emailJsTemplateId,
    ...
    console.log('✅ Notification de validation envoyée au client avec succès:', response)
    return { success: true, response }
  } catch (error) {
    ...
    // Ne pas bloquer la validation en cas d'erreur de notification
    return { success: false, error: error.message || 'Erreur inconnue' }
  }
}
